...
Your submission will be scored at the end of the round.
...
The scoring script for the """ + ismb_akes_challenge.CHALLENGE_NAME + """ encountered an error:
...
def validate_submission(evaluation, submission):
    raise NotImplementedError("validate_submission not implemented")
...
def score(evaluation,
    ...
    for submission, status in syn.getSubmissionBundles(evaluation, status='VALIDATED'):
        ...
        try:
            score, message = ismb_akes_challenge.score_submission(evaluation, submission)
            ...
            if notifications and ismb_akes_challenge.ADMIN_USER_IDS:
                ...
                if dry_run:
                    ...
                    response = syn.sendMessage(
                        userIds=ismb_akes_challenge.ADMIN_USER_IDS,
                        messageSubject=subject,
                        messageBody=message)
        ...
        if send_messages:
            ...
            if status.status == 'SCORED':
                message_body = scoring_message_template.format(
                    ...
                    submission_name=submission.name,
                    submission_id=submission.id)
                subject = "Submission to "+ismb_akes_challenge.CHALLENGE_NAME
            else:
...
def command_list(args):
    ...
    """
    if args.all:
        for queue_info in ismb_akes_challenge.evaluation_queues:
        ...
        list_evaluations(project=ismb_akes_challenge.CHALLENGE_SYN_ID)
    ...
    if args.all:
        for queue_info in ismb_akes_challenge.evaluation_queues:
            score(queue_info['id'],
                  send_messages=args.send_messages,
                  notifications=args.notifications,
                  leaderboard_table=queue_info.get('leaderboard_table', None),
                  dry_run=args.dry_run)
    elif args.evaluation:
        queue_info = ismb_akes_challenge.evaluation_queue_by_id[args.evaluation]
...
def command_leaderboard(args):
    ...
    if args.evaluation in ismb_akes_challenge.leaderboard_columns:
        leaderboard_cols = ismb_akes_challenge.leaderboard_columns[args.evaluation]
    else:
...
def main():
    ...
    try:
        ...
        if args.notifications:
            ...
            if args.dry_run:
                print "Dry Run: error notification:", "Exception while scoring " + ismb_akes_challenge.CHALLENGE_NAME
                print message
            else:
                response = syn.sendMessage(
                    userIds=ismb_akes_challenge.ADMIN_USER_IDS,
                    messageSubject="Exception while scoring " + ismb_akes_challenge.CHALLENGE_NAME,
                    messageBody=message)
                print "sent notification: ", unicode(response).encode('utf-8')
